...
import pandas as pd
import numpy as np
from organ_abnormality_list import abnormality_dict, finding_section_list
...
section_question_prompt = "What findings of <ABN_NAME> do you observe in this medical image?"
...
def gen_text(args):
    ...
    # model_LLM = LLM_pipline(model_id=phi3_model_path, max_new_tokens=512, device_map='cuda:0')
    ...
    ABN_pred_df = pd.read_excel(os.path.join(answer_path, 'vqa_output_pred.xlsx'), index_col=0) # VLM-VQA abnormality prediction
    # ABN_pred_df = pd.read_excel(os.path.join(answer_path, 'test_output_pred.xlsx'), index_col=0) # multi-label classifier abnormality prediction
    # ABN_pred_df = pd.read_excel(os.path.join(answer_path, 'test_multilabel_gt.xlsx'), index_col=0)  # abnormality labels from reports
    ...
    for accNum in tqdm(pd.unique(accNum_list)):            
        ...
        accNum_abn_pred_result_df = ABN_pred_df[ABN_pred_df['AccessionNumber_md5'] == accNum]
        acc_findings_text = ''
        for index, line in lines.iterrows():
            image_path = line["image"]
            img_modal = line["img_modal"]
            findings_gt =  line["Findings Text"]
            
            image_abn_pred_result_df = accNum_abn_pred_result_df[accNum_abn_pred_result_df['img_modal'] == img_modal]
            ...
            for section_name in finding_section_list:
                section_findings_dict[section_name] = {}
                for abn_name in abnormality_dict[section_name]:   
                    has_abn = image_abn_pred_result_df[[abn_name]].values[0,0] > 0
                    if has_abn:
                        qs = section_question_prompt.replace('<ABN_NAME>', abn_name)
                        outputs = ask_model(model, tokenizer, qs, image_tensor, use_image=True, max_new_tokens=48)
                    else:
                        outputs = ''
                    section_findings_dict[section_name][abn_name] = outputs
                    ...
                    ans_file_section.write(json.dumps({
                                ...
                                "findings_abnormality_pred": outputs,
                                # "model_id": model_name,
                                }) + "\n")
            ...
            for section_name in finding_section_list:    
                # print(section_name)
                image_findings_text += section_name + ': '
                section_text = ''
                for abn_name in abnormality_dict[section_name]:    
                    # print(abn_name + str(accNum_abn_sums[abn_name]) +':', section_findings_dict[section_name][abn_name])         
                    section_output_text = section_findings_dict[section_name][abn_name]
                    if len(section_output_text) > 0:
                        if len(section_output_text.split(' ')) < 3:
                            section_output_text = abn_name + ' is ' + section_output_text 
                        else:
                            section_output_text = section_output_text 
                        section_text += section_output_text + '. '
                if len(section_text.replace(' ', '')) == 0:
                    section_text = 'Normal.'
                image_findings_text += section_text + '\n'
...
if __name__ == "__main__":
    ...
    answer_path = './eval_output_abnormality_reports/'
